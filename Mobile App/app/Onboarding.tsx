...
export default function Onboarding() {
  ...
  // KEY FUNCTION: Save launch status and navigate
  ...
  const handleNext = () => {
    if (currentIndex < HeroSectionDetails.length - 1) {
      ...
      <View style={[styles.mainWrapper, { paddingTop: insets.top, paddingBottom: insets.bottom }]}>
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <View style={styles.slide}>
              <LinearGradient colors={['#16A085', '#f59e0b']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.iconCircle}>
                <FontAwesome5 name={item.icon} size={45} color="white" />
              </LinearGradient>

              <BlurView intensity={Platform.OS === 'ios' ? 50 : 100} tint="light" style={styles.glassCard}>
                <Text style={styles.header}>{item.header}</Text>
                <Text style={styles.description}>{item.description}</Text>
                
                <View style={styles.footerRow}>
                  {item.footer.map((tag, idx) => (
                    <View key={idx} style={[styles.tag, { backgroundColor: tag.backgroundColor }]}>
                ...
                {index === HeroSectionDetails.length - 1 && (
                  <TouchableOpacity activeOpacity={0.8} style={styles.ctaButton} onPress={completeOnboarding}>
        ...
        <View style={styles.bottomContainer}>
          ...
          <View style={styles.navRow}>
            <TouchableOpacity onPress={() => flatListRef.current?.scrollToIndex({ index: Math.max(0, currentIndex - 1) })}>
              <Text style={[styles.navText, { opacity: currentIndex === 0 ? 0 : 1 }]}>Prev</Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity onPress={handleNext}>
              <Text style={[styles.navText, { opacity: currentIndex === HeroSectionDetails.length - 1 ? 0 : 1 }]}>Next</Text>
            </TouchableOpacity>
          </View>
...
const styles = StyleSheet.create({
  ...
  logoCircle: { width: 34, height: 34, borderRadius: 17, justifyContent: 'center', alignItems: 'center' },
  logoText: { marginLeft: 10, fontSize: 18, fontWeight: '800', color: '#1e293b', letterSpacing: -0.5 },
  b2bBadge: { backgroundColor: 'rgba(255,255,255,0.6)', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12, borderWidth: 1, borderColor: 'rgba(255,255,255,0.8)' },
  b2bText: { fontSize: 10, fontWeight: '700', color: '#64748b' },
  subHeadline: { fontSize: 16, fontWeight: '600', color: '#475569', marginTop: 24, lineHeight: 22 },
  flatListContent: { alignItems: 'center' },
  slide: { width: width - 48, alignItems: 'center', justifyContent: 'center' },
  iconCircle: {
    width: 90, height: 90, borderRadius: 45, 
    justifyContent: 'center', alignItems: 'center',
    marginBottom: -45, zIndex: 10,
    shadowColor: '#16A085', shadowOpacity: 0.25, shadowRadius: 12, elevation: 8
  },
  glassCard: {
    width: '100%', padding: 24, paddingTop: 60, borderRadius: 32,
  ...
  ctaButton: { 
    backgroundColor: '#16A085', marginTop: 24, borderRadius: 16, 
  ...
  teaserRow: { 
    ...
    paddingTop: 20, borderTopWidth: 1, borderTopColor: 'rgba(226, 232, 240, 0.6)' 
  },
  teaserItem: { alignItems: 'center' },
  teaserIcon: { width: 42, height: 42, borderRadius: 21, justifyContent: 'center', alignItems: 'center' },
  ...
  actionLinks: { flexDirection: 'row', justifyContent: 'center', gap: 15, marginTop: 24 },
  linkText: { fontSize: 13, color: '#1e293b', fontWeight: '700' },
  skipText: { fontSize: 13, color: '#94a3b8', fontWeight: '500' },
  divider: { color: '#e2e8f0' },
  copyright: { textAlign: 'center', fontSize: 10, color: '#94a3b8', marginTop: 16, letterSpacing: 1 }
});